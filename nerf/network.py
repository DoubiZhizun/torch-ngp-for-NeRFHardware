...
class NeRFNetwork(NeRFRenderer):
    ...
    def save_hardware_parameters(self, path):
        ...
        with torch.no_grad():
            ...
            weight_now = torch.flip(torch.transpose(torch.clip(torch.round(self.sigma_net[1].weight * 1024).to(dtype=torch.int), -32768, 32767), 1, 0), [0]).reshape(-1).tolist()
            num = struct.pack('i', len(weight_now) * 2)
            fid.write(num)
            ...
            weight_now = torch.flip(torch.transpose(torch.concat([torch.zeros([64, 1], dtype=torch.int, device=weight_now.device), weight_now], 1), 1, 0), [0]).reshape(-1).tolist()
            num = struct.pack('i', len(weight_now) * 2)
            fid.write(num)
            num = struct.pack('2048h', *weight_now)
            fid.write(num)

            weight_now = torch.flip(torch.transpose(torch.clip(torch.round(self.color_net[1].weight * 1024).to(dtype=torch.int), -32768, 32767), 1, 0), [0]).reshape(-1).tolist()
            num = struct.pack('i', len(weight_now) * 2)
            fid.write(num)
            num = struct.pack('4096h', *weight_now)
            fid.write(num)

            weight_now = torch.clip(torch.round(self.color_net[2].weight * 1024), -32768, 32767).to(dtype=torch.int)
            weight_now = torch.flip(torch.transpose(torch.concat([weight_now, torch.zeros([1, 64], dtype=torch.int, device=weight_now.device)], 0), 1, 0), [0]).reshape(-1).tolist()
            num = struct.pack('i', len(weight_now) * 2)
            fid.write(num)
